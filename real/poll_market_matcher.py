...
def normalize_team(value: str) -> str:
    ...
    if alias:
        return alias
    # Some sportsbook feeds send "CAR Hurricanes"/"COL Avalanche" style team names.
    # Use the leading 3-letter code as a fallback before giving up.
    tokens = normalized.split()
    if len(tokens) >= 2:
        first_token = tokens[0]
        first_alias = TEAM_ALIASES.get(first_token)
        if first_alias and len(first_token) == 3:
            return first_alias
    return key
