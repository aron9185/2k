...
GAME_STAT = True
odds_file = BASE_DIR / 'picks.txt'
output_file = BASE_DIR / 'picks_ev.txt'
excel_file = '5reb'

if GAME_STAT:
    input_excel = RANK_DIR / f'game_{excel_file}.xlsx'
else:
    input_excel = RANK_DIR / f'{excel_file}.xlsx'
...
if 'pt' in excel_file:
    statType = 1
    value = excel_file[:-2]
elif 'ast' in excel_file:
    statType = 2
    value = excel_file[:-3]
elif 'reb' in excel_file:
    statType = 3
    value = excel_file[:-3]
elif '_3' in excel_file:
    statType = 21
    value = excel_file[:-2]
else:
    raise ValueError(f"Could not infer stat type from excel file key: {excel_file}")
...
def fetch_player_stat(player_id, statType, value):
    data = client.get_player_boxscore_splits(
        entity_id=player_id,
        entity_type="player",
        sport="nba",
        stat_type=statType,
...
player_info_df = pd.read_csv(BASE_DIR / "real_id.csv")  # Load "real_id.csv" containing player ID, Name
