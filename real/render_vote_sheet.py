...
def _binary_consensus_odds_pair(row: dict[str, str]) -> str:
    options = _option_slots(row)
    fair_probs = _binary_fair_probabilities(row)
    if len(options) != 2 or len(fair_probs) != 2:
        return ""
    ...
    for option in options:
        ...
        if fair_prob is None:
            return ""
        fair_odds = _format_american(str(probability_to_american(float(fair_prob))))
        if not fair_odds:
            return ""
        parts.append(f"{_compact_label(option['label'])} {fair_odds}")
    return " / ".join(parts)


def _compact_sportsbook_pair(row: dict[str, str]) -> str:
    consensus_pair = _binary_consensus_odds_pair(row)
    if consensus_pair:
...
def _resolved_game_order(row: dict[str, str]) -> int:
    game_order = _safe_int(str(row.get("game_order") or ""))
    return game_order if game_order is not None else 999999


def _render_poll_sort_key(row: dict[str, str]) -> tuple[int, int, int, int, int, str]:
    post_order = _safe_int(str(row.get("post_order") or ""))
    source_order = _safe_int(str(row.get("_source_order") or ""))
    header = str(row.get("header") or "").strip()
    poll_kind = str(row.get("poll_kind") or "").strip()
    return (
        _resolved_game_order(row),
        post_order if post_order is not None else 999999,
        source_order if source_order is not None else 999999,
        MLB_POLL_KIND_ORDER.get(poll_kind, 99),
        POLL_ORDER.get(header, 99),
        header,
    )
...
def render_vote_sheet(
    ...
    *,
    ...
    for source_order, row in enumerate(rows):
        cloned = dict(row)
        cloned["_source_order"] = str(source_order)
        indexed_rows.append(cloned)
    rows = sorted(indexed_rows, key=_render_poll_sort_key)

    grouped: dict[tuple[str, str, str, str, str, str], list[dict[str, str]]] = defaultdict(list)
    ordered_game_keys: list[tuple[str, str, str, str, str, str]] = []
    for row in rows:
        key = (
            str(_resolved_game_order(row)),
    ...
    for key in ordered_game_keys:
        game_rows = grouped[key]
        ...
        rows_for_game = sorted(game_rows, key=_render_poll_sort_key)
        sections.append("")
